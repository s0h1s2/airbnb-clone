<template>
  ...
  <EmptyState v-else-if="trips === null || trips?.length == 0" title="No reservations found"
    subtitle="Your properties rented by nobody" :showReset="false" />
  <Container v-else>
    ...
    <div class="
      ...
      <ListingCard v-for="trip in trips" :listing="trip"
        :reservation="{ startDate: trip.startDate, endDate: trip.endDate }" actionLabel="Cancel guest trip"
...
client.get<OkResponseResult<Trips[]>>("/listing/reservations").then((r) => {
  trips.value = r.data.data
}).catch(() => {
  toast.warning("Unable to load reservations.")
}).finally(() => {
